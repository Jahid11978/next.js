...
impl<B: BackingStorage> TurboTasksBackendInner<B> {
    ...
    fn debug_get_task_description(&self, task_id: TaskId) -> String {
        ...
        if let Some(CachedDataItemValueRef::TaskType { value }) =
            task.get(&CachedDataItemKey::TaskType {})
        {
            format!("{task_id:?} {}", value)
        } else if let Some(CachedDataItemValueRef::TransientTaskType { value }) =
            task.get(&CachedDataItemKey::TransientTaskType {})
        {
            format!("{task_id:?} {}", value)
        } else {
    ...
    fn task_execution_completed_invalidate_output_dependent(
        ...
        fn process_output_dependents(
            ...
            let transient_task_type = get!(dependent, TransientTaskType);
    ...
    fn create_transient_task(&self, task_type: TransientTaskType) -> TaskId {
        ...
        };
        {
            ...
            task.add(CachedDataItem::TransientTaskType {
                value: Arc::new(match task_type {
            ...
            task.add(CachedDataItem::new_scheduled(
                ...
                EventDescription::new(|| move || root_type.to_string()),
            ));
        }
        #[cfg(feature = "verify_aggregation_graph")]
